        ...
        try {
            if (q.Priority == 'high' && (q.Cases == null || q.Cases.size() == 0) && Trigger.oldMap.get(q.id).Priority != 'high') {
                ...
                if (Test.isRunningTest()) {
                    newCase.Initial_Reason_For_Contact__c = 'Buy or Book';
                    newCase.Journey_Type_Product__c = 'Disney';
                    //newCase.Category__c = 'Pre Travel';
                    /*newCase.Topic__c = 'Lounge';
                    newCase.What_Is_The_Question__c = 'What Do I Need?';*/
                    newCase.Technical_Issue__c = 'No points availability';
                }
                insert newCase;
            }
        } catch (Exception e) {
