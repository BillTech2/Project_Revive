trigger PreferenceTrigger on Preference__c (before insert, after insert, after update) {
    if (DLT_Automation_Settings__c.getOrgDefaults().Disable_Preference_Automation__c) { return; }
    ...
    if (Trigger.isAfter) {
        if(Trigger.isInsert){
            ...
            if (PreferenceTriggerHandler.accountsToUpdate.values().isEmpty() == FALSE) {
                update PreferenceTriggerHandler.accountsToUpdate.values();
            }
    
            if (PreferenceTriggerHandler.preferenceToUpdate.values().isEmpty() == FALSE) {
